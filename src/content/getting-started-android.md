...
There's a total of 3 different use cases (click the links to jump to their section). You can use the provided [`MaterialIconView`](http://dev.materialdesignicons.com/getting-started/android#materialiconview) which mostly is just a more advanced `ImageView` or use your preferred `ImageView` and use the [`MaterialDrawable`](http://dev.materialdesignicons.com/getting-started/android#materialdrawable) as Drawable resource. If you want to spice up your `Toolbar` with icons from this library there is a custom [`MaterialMenuInflater`](http://dev.materialdesignicons.com/getting-started/android#materialmenuinflater) that does just that in a single line of code.
