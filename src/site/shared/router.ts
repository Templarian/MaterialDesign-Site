 ...
 * @returns {Router}
 */
export function Router(options): void {
...
Router.prototype.redirectTo = function (path, state, silent) {
  ...
  if (this.mode === "history") {
    history.replaceState(state, '', this.root + this._trimSlashes(path));
    return this.check();
  } else {
    this._historyIdx--;
...
Router.prototype.navigateTo = function (path, state, silent) {
  ...
  if (this.mode === "history") {
    history.pushState(state, '', this.root + this._trimSlashes(path));
    return this.check();
  } else {
    window.location.hash = path;
  }
